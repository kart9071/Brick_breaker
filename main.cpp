...
LPCWSTR filepath_hit = L"ball_hit.wav";
LPCWSTR filepath_win = L"gameover.wav.wav";
...
GLfloat brick_color_array[][3] = { {1,0,0},{0,0,1},{0,1,0},{1,1,1},{1,1,0} };
...
bool checkBrickCollision(float bx, float by, float brickX, float brickY) {
	float brickWidth = 3.0;
	float brickHeight = 1.0;

	if (bx + 1.0 >= brickX && bx - 1.0 <= brickX + brickWidth &&
		by + 1.0 >= brickY && by - 1.0 <= brickY + brickHeight) {
		return true;
	}
...
bool checkPaddleCollision(float bx, float by, float paddleX, float paddleY) {
	float paddleWidth = paddle_size[size];
	float paddleHeight = 1.0;

	if (bx + 1.0 >= paddleX - paddleWidth && bx - 1.0 <= paddleX + paddleWidth &&
		by - 1.0 <= paddleY + paddleHeight && by - 1.0 >= paddleY) {
		return true;
	}
...
void handleCollision() {
	for (int i = 1; i <= rows; i++) {
		for (int j = 1; j <= columns; j++) {
			if (brick_array[i][j].x != 0 && brick_array[i][j].y != 0) {
				if (checkBrickCollision(bx, by, brick_array[i][j].x, brick_array[i][j].y)) {
					brick_array[i][j].x = 0;
					brick_array[i][j].y = 0;
					score++;
					PlaySound(filepath_hit, NULL, SND_ASYNC);
					diry = -diry;
				}
			}
		}
	...
	if (checkPaddleCollision(bx, by, px, -15.0)) {
		diry = -diry;
		PlaySound(filepath_hit, NULL, SND_ASYNC);
	}
}
...
void text(int sc)
{
	...
	if (dirx > 0)
	{
		sprintf_s(direction, "Paddle moving right");
	}
	else if (dirx < 0)
	{
		sprintf_s(direction, "Paddle moving left");
	}
	else
	{
		sprintf_s(direction, "Paddle stationary");
	}
	if (sc < 40)
	...
	//postition for the direction 
	glPushMatrix();
	glTranslatef(-0.7, -0.5, 0);  // Adjust the x-coordinate to position the direction text
	glRasterPos3f(0, 0, 20);
	for (int i = 0; direction[i] != '\0'; i++)
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, direction[i]);
	glPopMatrix();
	glEnable(GL_LIGHTING);
}
...
//this function hits the brick and get small changes in the direction.
//After hitting the brick that bricks x and y coordinates set to 0
void hit()
{
	int i, j;
	for (i = 1;i <= rows;i++)
		for (j = 1;j <= columns;j++)
		{
			if ((bx >= brick_array[i][j].x - 19.5 - 0.1) && (bx <= brick_array[i][j].x + 3 - 19.5 + 0.1))
			{
				...
				{
					PlaySound(filepath_hit, NULL, SND_FILENAME | SND_ASYNC);
					brick_array[i][j].x = 0;
					brick_array[i][j].y = 0;
					diry = diry * -1;
					score++;
...
int main(int argc, char** argv) {
	...
	// The audio playback started successfully
